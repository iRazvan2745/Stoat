...
it("preserves waiting and healthy active jobs regardless of age or log silence", () => {
    for (const state of ["waiting", "delayed", "active", "waiting-children"]) {
        expect(
            deploymentRecoveryReason(state, queuedAt, dayLater)
        ).toBeUndefined();
    }
});
