...
describe("getPhase", () => {
  ...
  it("returns queued, started, then finished as timestamps appear", () => {
    expect(getPhase(deployment({ queuedAt: new Date("2026-08-19T12:01:00.000Z") }))).toBe("queued");
    expect(
      getPhase(
        deployment({
...
describe("getDisplayStatus", () => {
  ...
  it("marks a finished deployment as deployed", () => {
    expect(getDisplayStatus(deployment({ finishedAt: new Date("2026-08-19T12:03:00.000Z") }))).toBe(
      "deployed",
    );
  });
...
describe("getDuration", () => {
  it("returns null until both start and finish exist", () => {
    expect(getDuration(deployment({ startedAt: new Date("2026-08-19T12:02:00.000Z") }))).toBeNull();
  });
