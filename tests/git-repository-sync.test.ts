...
const commitVersions = async (
    ...
    return commitVersions(repo, root, versions, index + 1, commits);
};
...
it("enumerates every unseen commit in order and reads each immutable Compose version", async () => {
    ...
    try {
        ...
        const commits = await commitVersions(repo, root, [1, 2, 3]);
        expect(
            await pendingCommits(repo, commits[0] ?? null, commits[2] ?? null)
        ).toEqual(commits.slice(1));
        expect(await pendingCommits(repo, null, commits[2] ?? null)).toEqual(
            commits.slice(2)
        );
        ...
        expect(
            await readFile(path.join(root, "prod/web/compose.yaml"), "utf-8")
        ).toContain("nginx:3");
...
it("refuses to write through a repository file or directory symlink", async () => {
    ...
    try {
        ...
        await expect(
            writeRepositoryFile(root, "linked/secret", "overwrite")
        ).rejects.toThrow("symlink");
        await expect(
            writeRepositoryFile(root, "compose.yaml", "overwrite")
        ).rejects.toThrow("symlink");
        expect(
            await readFile(path.join(root, "outside", "secret"), "utf-8")
        ).toBe("unchanged");
    } finally {
        await rm(root, { force: true, recursive: true });
    }
...
it("stages literal repository paths without including neighboring files through Git wildcards", async () => {
    ...
    try {
        ...
        const tree = await repo.raw(["ls-tree", "-r", "--name-only", "HEAD"]);
        expect(tree.trim()).toBe("prod/*/compose.yaml");
    } finally {
        await rm(root, { force: true, recursive: true });
    }
});
