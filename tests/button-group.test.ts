...
const source = readFileSync(
    new URL(
        "../node_modules/m3-svelte/package/buttons/ButtonGroup.svelte",
        import.meta.url
    ),
    "utf-8"
);
...
describe("ButtonGroup spec tokens", () => {
    ...
    it("uses XS 4 / S 8 / M 8 / L 16 / XL 20 inner corners", () => {
        ...
        expect(source).toMatch(
            /\.size-s \{[\s\S]*?--inner-corner: var\(--m3-shape-small\);/u
        );
        expect(source).toMatch(
            /\.size-m \{[\s\S]*?--inner-corner: var\(--m3-shape-small\);/u
        );
        expect(source).toMatch(
            /\.size-l \{[\s\S]*?--inner-corner: var\(--m3-shape-large\);/u
        );
        expect(source).toMatch(
    ...
    it("rounds only the outer ends of a connected round group", () => {
        ...
        expect(source).not.toContain(
            "border-radius: calc(var(--button-group-min) / 2)"
        );
        expect(source).not.toContain(
            "border-start-start-radius: var(--m3-shape-medium)"
        );
    });
