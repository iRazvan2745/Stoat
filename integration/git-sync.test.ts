...
const { db } = await import("#lib/db");
const { dataSource, gitSource, organization, services, workspace } = await import("#lib/db/schema");
...
const { publishServiceDeployment, syncGitSource } = await import("#lib/server/git-sources/sync");
...
it("syncs both directions, preserves each commit snapshot, and does not enqueue duplicates", async () => {
    ...
    const composePath = `${wrk.slug}/${service.slug}/compose.yaml`;
    await writeRepositoryFile(author, composePath, exportGitCompose(service, wrk));
    await repo.add(".");
    ...
    for (const version of [2, 3]) {
        await writeRepositoryFile(
            ...
            composePath,
            exportGitCompose(
                { ...service, value: service.value?.replace("nginx:1", `nginx:${version}`) ?? "" },
                wrk,
            ),
        );
        await repo.add(".");
    ...
    const snapshots = fixture.snapshots.filter((snapshot) => snapshot.service.id === service.id);
    expect(snapshots.slice(-2).map((snapshot) => snapshot.gitCommit)).toEqual(revisions);
    expect(snapshots.at(-2)?.service.value).toContain("nginx:2");
    expect(snapshots.at(-1)?.service.value).toContain("nginx:3");
    const [updated] = await db.select().from(services).where(eq(services.id, service.id));
    expect(updated?.value).toContain("  web:");
    expect(updated?.value).not.toContain(`${service.slug}-web:`);
    ...
    await db
        .update(services)
        .set({ value: service.value?.replace("nginx:1", "nginx:4") })
    ...
    await publishServiceDeployment(snapshot);
    ...
    await writeRepositoryFile(
        ...
        composePath,
        exportGitCompose(
            { ...service, value: service.value?.replace("nginx:1", "nginx:5") ?? "" },
            wrk,
        ),
    );
    await repo.add(".");
    ...
    expect(fixture.snapshots.at(-1)?.service.value).toContain("nginx:5");
    // Concurrent edits stop at the conflicting commit without overwriting the app.
    await db
        .update(services)
        .set({ value: service.value?.replace("nginx:1", "nginx:local") })
        .where(eq(services.id, service.id));
    await writeRepositoryFile(
        author,
        composePath,
        exportGitCompose(
            { ...service, value: service.value?.replace("nginx:1", "nginx:remote") ?? "" },
            wrk,
        ),
    );
    await repo.add(".");
    ...
    const [preserved] = await db.select().from(services).where(eq(services.id, service.id));
    ...
    expect(fixture.snapshots.at(-2)?.service.value).toContain("nginx:remote");
    expect(fixture.snapshots.at(-1)?.service.value).toContain("nginx:local");
    expect((await syncGitSource(source.id)).deployments).toBe(0);
});
...
it("imports a structured repository into matching workspace and group folders once", async () => {
    ...
    const [web] = await db
        .select()
        .from(services)
        .where(eq(services.workspaceId, production?.id ?? ""));
    ...
    if (!web) throw new Error("Missing imported service");
    await db.delete(services).where(eq(services.id, web.id));
    await repo.pull(["--ff-only"]);
    await writeRepositoryFile(author, "README.md", "Another repository change\n");
    await repo.add("README.md");
    await repo.commit("Update documentation after deleting the app service");
    await repo.push();
...
it("initializes an empty remote from app configuration and serializes concurrent syncs", async () => {
    ...
    expect(files).toContain(`${wrk.slug}/Tools/${service.slug}/compose.yaml`);
    await db.update(services).set({ groupName: "Utilities" }).where(eq(services.id, service.id));
    expect((await syncGitSource(source.id)).issues).toEqual([]);
    const movedFiles = await simpleGit(remote).raw(["ls-tree", "-r", "--name-only", "HEAD"]);
    expect(movedFiles).toContain(`${wrk.slug}/Utilities/${service.slug}/compose.yaml`);
    ...
    await repo.mv(
        `${wrk.slug}/Utilities/${service.slug}`,
        `${wrk.slug}/Operations/${service.slug}`,
    );
    await repo.commit("Move group in Git");
    await repo.push();
    expect((await syncGitSource(source.id)).issues).toEqual([]);
    const [movedService] = await db.select().from(services).where(eq(services.id, service.id));
    expect(movedService?.groupName).toBe("Operations");
    expect((await syncGitSource(source.id)).deployments).toBe(0);
});

it("queues separate commits for the same service while the first deployment is still running", async () => {
    const queue = await vi.importActual<typeof import("#lib/server/deployments/deployments")>(
        "#lib/server/deployments/deployments",
    );
    const serviceId = fixture.snapshots.at(-1)?.service.id;
    if (!serviceId) throw new Error("Missing queue fixture");
    const snapshot = await captureDeploymentSnapshot(serviceId);
