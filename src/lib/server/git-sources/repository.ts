...
export const pendingCommits = async (
    ...
    try {
        const mergeBase = await repo.raw(["merge-base", previous, head]);
        const ancestor = mergeBase.trim();
        if (ancestor !== previous) {
            throw new GitSyncError("Unrelated history");
        }
    } catch {
...
export const readCommitComposeFiles = async (
    ...
    for (const entry of tree.split("\0")) {
        ...
        if (relativePath.split("/").some((part) => IGNORED.has(part))) {
            continue;
        }
        ...
        if (compose === undefined) {
            ...
            if (size > 1_048_576) {
                throw new GitSyncError(
                    `Compose file exceeds 1 MiB: ${relativePath}`
                );
            }
...
export const readCommitSiblingFiles = async (
    ...
    const wanted = new Set(
        relativePaths
            .map((candidate) =>
                path.posix.normalize(candidate.replace(/^(?:\.\/)+/u, ""))
            )
            .filter(
                (candidate) =>
                    candidate !== "" &&
    ...
    for (const entry of tree.split("\0")) {
        ...
        const [mode, type, hash] = entry.slice(0, tab).split(" ");
        const relativePath = entry.slice(tab + 1);
        if (type !== "blob" || !mode?.startsWith("100") || !hash) {
            continue;
        }
        ...
        const content = await readCachedBlob(
            repo,
            hash,
            cache,
...
export const writeRepositoryFile = async (
    ...
    await fs.writeFile(current, contents, "utf-8");
};
