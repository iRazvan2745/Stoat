...
import type { ContainerLogRecord, ContainerLogSource } from "#lib/domain/services/container-logs";
import {
    insertLogSorted,
    matchContainerId,
...
import type { ServiceContainerInfo } from "#lib/server/services/service-containers";
import { listServiceContainers } from "#lib/server/services/service-containers";
...
const buildServiceLogsUrl = (connection: UncloudConnection, uncloudServiceId: string): string => {
...
const openServiceLogStream = async (
    ...
    try {
        const response = await fetch(buildServiceLogsUrl(connection, uncloudServiceId), {
            headers: {
...
const prepareServiceLogContext = async (serviceId: string): Promise<PreparedServiceLogs> => {
    ...
    return {
        connection: await getServiceDataSource(serviceId),
    ...
    try {
        ...
        const connections = await Promise.allSettled(
            serviceNames.map((uncloudServiceId) =>
                openServiceLogStream(connection, uncloudServiceId, controller.signal),
            ),
        );
