...
export async function prepareDeployment(
    ...
    const gitSync = gitUrl
        ? withKeyedLock(`workspace-git:${wrk.id}`, async () => {
              const repo = await getRepo({ repoPath, repoUrl: gitUrl });
    ...
    const deploy = await createUncloudStreamClient(ds).POST("/api/v1/services/deploy/compose", {
        body: {
