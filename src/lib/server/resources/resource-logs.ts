...
export interface ResourceContainerLogsSnapshot {
...
type PreparedResourceLogs =
    | { ok: false; snapshot: ResourceContainerLogsSnapshot }
    | {
          ok: true;
          connection: UncloudConnection;
          containers: ContainerLogSource[];
...
const prepareResourceLogContext = async (resourceId: string): Promise<PreparedResourceLogs> => {
    ...
    if (!resource) {
        return {
            ok: false,
            snapshot: snapshotOf([], [], false, "Resource not found"),
        };
    }
    ...
    try {
        formatted = formatComposeFile(resource.value, prefix);
    } catch {
        return {
            ok: false,
    ...
    const listed = await listResourceContainers(resourceId);
...
export async function* streamResourceContainerLogs(
    ...
    try {
        const prepared = await prepareResourceLogContext(resourceId);
