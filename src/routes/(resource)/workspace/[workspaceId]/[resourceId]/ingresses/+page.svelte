<script lang="ts">
    ...
    import {
        createResourceIngress,
        deleteResourceIngress,
        getResource,
        listResourceIngresses,
        updateResourceIngress,
    ...
    import type {
        ResourceIngress,
        ResourceIngressProtocol,
    } from "#lib/domain/resources/ingresses";
    import { copyToClipboard } from "#lib/shared/ui/clipboard";

    const { params } = $props();

    const resource = $derived(getResource(params.resourceId));
    const ingressQuery = $derived(listResourceIngresses(params.resourceId));

    const svc = $derived(await resource);
    ...
    let editingRoute = $state<ResourceIngress>();
    let deletingRoute = $state<ResourceIngress>();
    let composeService = $state("");
    let containerPort = $state("");
    let hostname = $state("");
    let protocol = $state<ResourceIngressProtocol>("https");
    ...
    const openEditRoute = (route: ResourceIngress): void => {
        editingRoute = route;
    ...
    const routeIcon = (route: ResourceIngress) => {
        if (route.mode === "host") {
    ...
    const routeHeadline = (route: ResourceIngress): string => {
        if (route.url) {
            return route.url;
        }
    ...
    const routeTarget = (route: ResourceIngress): string =>
        `${route.composeService}:${route.containerPort}`;

    const copyable = (route: ResourceIngress): string | undefined => {
        if (route.url) {
            return route.url;
        }
    ...
    const modeClasses = (route: ResourceIngress): string =>
        route.mode === "ingress"
    ...
    const routeKey = (route: ResourceIngress): string =>
        [
            route.mode,
            route.host ?? "",
            route.path ?? "",
    ...
    <header class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex min-w-0 flex-col gap-1">
            ...
            <p class="m3-font-body-medium text-on-surface-variant">
                How traffic reaches this resource through Caddy and published
                ports.
            </p>
        </div>
    ...
    {:else}
        <Card variant="elevated">
            ...
            <div
                ...
                role="group"
                aria-label="Traffic flow diagram showing how public hosts and ports route to resource containers"
            >
...
>
    <div class="flex min-w-72 flex-col gap-4">
        <p class="m3-font-body-medium text-on-surface-variant">
            This updates the resource's <code class="font-mono">x-ports</code>
            in Compose. Deploy the resource when you're ready to apply it.
        </p>
        <SelectOutlined
        ...
        <p class="m3-font-body-small text-on-surface-variant">
            ...
            {:else}
                Leave the domain blank to use the cluster-assigned resource
                domain.
            {/if}
        </p>
    </div>
...
>
    <div class="flex flex-col gap-2">
        ...
        <p class="m3-font-body-small text-on-surface-variant">
            This does not redeploy the resource.
        </p>
    </div>
