...
<div class="mb-3 flex flex-wrap items-center justify-between gap-2">
    <div class="flex items-center gap-2">
        <span
            class="bg-surface-container text-on-surface-variant m3-font-label-medium inline-flex items-center gap-1.5 rounded-full px-2.5 py-1"
        >
            <span
                class="live-dot size-1.5 rounded-full {live
                    ? 'bg-tertiary'
                    : 'bg-primary'}"
                aria-hidden="true"
            ></span>
        ...
        <span class="text-on-surface-variant m3-font-label-small tabular-nums">
    ...
    {#if !followLatest}
        <Button variant="tonal" onclick={jumpToLatest}>
...
<style>
    .live-dot {
        animation: live-pulse var(--m3-duration-slow)
            var(--m3-timing-function-slow) infinite alternate;
    }
