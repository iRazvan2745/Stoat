<script lang="ts">
    ...
    import { getPostgresConnection } from "#lib/api/resources.remote";
    import {
        maskPostgresUrl,
        wrapUrlSegments,
    ...
    const connectionQuery = $derived(getPostgresConnection(resourceId));
...
{#snippet copyUrl(label: string, displayValue: string, copyValue: string)}
    <li>
        <button
            type="button"
            class="copy-button m3-layer text-on-surface grid w-full cursor-pointer grid-cols-[minmax(0,1fr)_2.5rem] items-center gap-x-2 gap-y-1 rounded-md border-0 bg-transparent px-1 py-2 text-left"
            title={copyValue}
