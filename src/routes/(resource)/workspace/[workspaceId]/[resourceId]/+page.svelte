<script lang="ts">
    ...
    // svelte-ignore state_referenced_locally
    const containers = listResourceContainers(params.resourceId);

    const svc = await resource;
    ...
    const logsHref = (containerId: string): string =>
        `/workspace/${params.workspaceId}/${params.resourceId}/logs?containers=${containerId}`;
    ...
    const resourceHealth = $derived.by(() => {
    ...
    const deploy = async (): Promise<void> => {
        ...
        try {
            const deployment = await deployResource(svc.id);
            await goto(
                `/workspace/${params.workspaceId}/${params.resourceId}/deployments?view=${deployment.deploymentId}`
            );
...
{#if svc}
    ...
    >
        ...
        >
            ...
            <div class="flex shrink-0 items-center gap-2">
                ...
                <div class="relative">
                    <Button
                        variant="text"
                        id="resource-actions-menu-button"
                        square
        ...
        >
            ...
            >
                {#if svc.type === "postgresql"}
                    <div class="flex-none [&>.m3-container]:w-full">
                        <DatabaseConnection resourceId={svc.id} />
                    </div>
                {/if}
                    ...
                    <Card variant="elevated">
                        ...
                        {:else if containerItems.length === 0}
                            ...
                            >
                                ...
                                <p class="m3-font-body-small">
                                    Deploy this resource to start them.
                                </p>
                            </div>
                        {:else}
                                ...
                                {#each containerItems as container (container.id)}
                                    <li class="min-w-0 flex-1">
                                        <a
                                            ...
                                            class="container-link m3-layer flex w-full min-w-0 items-center gap-3 rounded-md px-2 py-2"
            ...
            >
                <ComposeEditor resourceId={svc.id} initialCompose={svc.value} />
            </div>
        </div>
    </div>
{/if}
