...
const (
	// DefaultUncloudSockPath is the current Uncloud API socket, under a
	// dedicated parent directory so that directory can be bind-mounted.
	DefaultUncloudSockPath = "/run/uncloud/api/uncloud.sock"
	// LegacyUncloudSockPath is the previous Uncloud API socket path. Kept so
	// sidecars still start on machines that have not been upgraded.
	LegacyUncloudSockPath = "/run/uncloud/uncloud.sock"
)

// DefaultUncloudSockPaths is the current socket first, then the legacy path.
func DefaultUncloudSockPaths() []string {
	return []string{DefaultUncloudSockPath, LegacyUncloudSockPath}
}

// IsDefaultUncloudSockPath reports whether path is a well-known Uncloud API socket.
func IsDefaultUncloudSockPath(path string) bool {
	return path == DefaultUncloudSockPath || path == LegacyUncloudSockPath
}

// UnixSpecs returns unix connection specs for path. A well-known Uncloud socket
// also includes the other default path so old and new layouts both work.
func UnixSpecs(path string) []Spec {
	specs := make([]Spec, 0, 2)
	...
	add := func(p string) {
		...
		seen[p] = struct{}{}
		specs = append(specs, Spec{Unix: p})
	}
	add(path)
	if IsDefaultUncloudSockPath(path) || path == "" {
		add(DefaultUncloudSockPath)
		add(LegacyUncloudSockPath)
	}
	return specs
}

// ExistingUnixSpecs returns specs for whichever well-known Uncloud sockets exist.
func ExistingUnixSpecs() []Spec {
	var specs []Spec
	for _, path := range DefaultUncloudSockPaths() {
		if Exists(path) {
			specs = append(specs, Spec{Unix: path})
		}
	}
	return specs
}
...
func LoadConfigSpecs(path, contextName string) ([]Spec, error) {
	...
	for _, conn := range cluster.Connections {
		...
		if spec.Unix != "" && IsDefaultUncloudSockPath(spec.Unix) {
			specs = append(specs, UnixSpecs(spec.Unix)...)
			continue
		}
