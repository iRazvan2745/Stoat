...
func TestParseConnectFlag(t *testing.T) {
	tests := []struct {
		...
		{name: "unix current", value: "unix:///run/uncloud/api/uncloud.sock", expected: Spec{Unix: "/run/uncloud/api/uncloud.sock"}},
		{
			name:  "tcp",
...
`), 0o600))
	...
	assert.Equal(t, "/run/uncloud/api/uncloud.sock", specs[1].Unix)
	assert.Equal(t, "root@example.com", specs[2].SSH)
...
func TestLoadConfigSpecsKeepsCustomUnixSocket(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "config.yaml")
	require.NoError(t, os.WriteFile(path, []byte(`
current_context: appa
...
func TestUnixSpecs(t *testing.T) {
	t.Run("legacy includes current", func(t *testing.T) {
		specs := UnixSpecs(LegacyUncloudSockPath)
		require.Len(t, specs, 2)
		assert.Equal(t, LegacyUncloudSockPath, specs[0].Unix)
		assert.Equal(t, DefaultUncloudSockPath, specs[1].Unix)
	})
	t.Run("current includes legacy", func(t *testing.T) {
		specs := UnixSpecs(DefaultUncloudSockPath)
		require.Len(t, specs, 2)
		assert.Equal(t, DefaultUncloudSockPath, specs[0].Unix)
		assert.Equal(t, LegacyUncloudSockPath, specs[1].Unix)
	})
	t.Run("empty uses both defaults", func(t *testing.T) {
		specs := UnixSpecs("")
		require.Len(t, specs, 2)
		assert.Equal(t, DefaultUncloudSockPath, specs[0].Unix)
		assert.Equal(t, LegacyUncloudSockPath, specs[1].Unix)
	})
	t.Run("custom path is unchanged", func(t *testing.T) {
		specs := UnixSpecs("/tmp/custom.sock")
		require.Len(t, specs, 1)
		assert.Equal(t, "/tmp/custom.sock", specs[0].Unix)
	})
}
