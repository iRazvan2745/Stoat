...
func (s *Server) requireBearerToken(c *fiber.Ctx) error {
	...
	if !ok || !s.tokenMatches(token) {
		...
		return writeError(c, fiber.NewError(fiber.StatusUnauthorized, "a valid bearer token is required"))
	}
	return c.Next()
}
...
func (s *Server) withRequestDeadline(c *fiber.Ctx) error {
	ctx, cancel := context.WithTimeout(requestContext(c), s.requestTimeout)
	defer cancel()
...
func (s *Server) ucInternalMetrics(c *fiber.Ctx) error {
	...
	// Fiber reads the response body after the handler returns and the request
	// middleware cancels its context. Keep this bounded context until body close.
	metricsCtx, cancel := context.WithTimeout(detachedContext(c), s.requestTimeout)
	request, err := http.NewRequestWithContext(metricsCtx, http.MethodGet, targetURL, nil)
	if err != nil {
		cancel()
	...
	return c.SendStream(&metricsResponseStream{ReadCloser: response.Body, cancel: cancel})
}

type metricsResponseStream struct {
	io.ReadCloser
	cancel context.CancelFunc
}

func (s *metricsResponseStream) Close() error {
	defer s.cancel()
...
func machineMetricsIP(machine api.MachineMember) (netip.Addr, error) {
	...
	if !subnet.IsValid() || !subnet.Addr().Is4() {
		return netip.Addr{}, fiber.NewError(
			fiber.StatusServiceUnavailable,
			"local Uncloud machine has no cluster IPv4 subnet",
		)
	}
	...
	if !machineIP.IsValid() || !subnet.Contains(machineIP) {
		return netip.Addr{}, fiber.NewError(
			fiber.StatusServiceUnavailable,
			"local Uncloud machine has no usable cluster IPv4 address",
		)
	}
...
func (s *Server) serviceLogs(c *fiber.Ctx) error {
	...
	streamCtx, cancel := streamContext(c)
	_, entries, err := s.backend.ServiceLogs(streamCtx, c.Params("id"), opts)
	if err != nil {
		cancel()
		return writeError(c, err)
	}
	...
	return streamSSE(c, cancel, entries, encodeLogEvent)
}
...
func (s *Server) deployCompose(c *fiber.Ctx) error {
	...
	return streamSSE(c, cancel, events, encodeDeployEvent)
}
...
func (s *Server) execContainer(c *fiber.Ctx) error {
	...
	exitCode, err := s.backend.ExecContainer(execCtx, c.Params("id"), c.Params("container"), api.ExecOptions{
...
// detachedContext drops the per-request deadline set by withRequestDeadline.
// Handlers that keep working after they return (event streams) or that are
// expected to run longer than a normal request (command execution) need their
// own lifetime.
func detachedContext(c *fiber.Ctx) context.Context {
	return context.WithoutCancel(requestContext(c))
}

// streamContext returns the context for an event stream. The returned cancel
// function is owned by the stream writer, which calls it when the stream ends.
func streamContext(c *fiber.Ctx) (context.Context, context.CancelFunc) {
	return context.WithCancel(detachedContext(c))
}

// execContext bounds a command execution by the configured exec timeout.
func (s *Server) execContext(c *fiber.Ctx) (context.Context, context.CancelFunc) {
	return context.WithTimeout(detachedContext(c), s.execTimeout)
}
