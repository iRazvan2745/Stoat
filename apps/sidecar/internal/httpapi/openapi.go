...
// OpenAPISecurityRequirement maps a security scheme name to its required scopes.
	...
	document := OpenAPIDocument{
		...
		Info: OpenAPIInfo{
			...
			Description: "REST API for managing Uncloud clusters. " +
				"Every endpoint except /healthz and /docs requires the sidecar's bearer token.",
			Version: "v1",
		},
		Servers:  []OpenAPIServer{{URL: "/", Description: "Current Uncloud API server"}},
		Security: []OpenAPISecurityRequirement{{bearerSchemeName: {}}},
		Tags: []OpenAPITag{
		...
		Components: OpenAPIComponents{
			...
			SecuritySchemes: map[string]OpenAPISecurityScheme{
				bearerSchemeName: {
					Type:   "http",
					Scheme: "bearer",
					Description: "Bearer token configured through the sidecar's SIDECAR_TOKEN " +
...
// bearerSchemeName is the name of the only security scheme in the document.
const bearerSchemeName = "bearerAuth"

// noSecurity marks the operations that are reachable without a bearer token.
// An empty (non-nil) slice is what OpenAPI uses to clear the document-level
// requirement, so this must not be replaced with nil.
func noSecurity() *[]OpenAPISecurityRequirement {
	requirements := []OpenAPISecurityRequirement{}
	return &requirements
}

// makePublic marks an operation as reachable without credentials. It also drops
// the inherited 401, which such an operation cannot return; leaving it in place
// would document a rejection that never happens.
func makePublic(operation *OpenAPIOperation) {
...
func openAPIPaths() map[string]OpenAPIPathItem {
	paths := map[string]OpenAPIPathItem{
		...
		"/openapi.json": {
			Get: operation("docs", "Get this OpenAPI document as JSON", "openapiJSON", nil, nil,
				map[string]OpenAPIResponse{
					"200": mediaResponse("OpenAPI document", "application/json", OpenAPISchema{Type: "object"}),
					"401": response("Missing or invalid bearer token", ref("ErrorResponse")),
				}),
		},
		"/openapi.yaml": {
			Get: operation("docs", "Get this OpenAPI document as YAML", "openapiYAML", nil, nil,
				map[string]OpenAPIResponse{
					"200": mediaResponse("OpenAPI document", "application/yaml", OpenAPISchema{Type: "string"}),
					"401": response("Missing or invalid bearer token", ref("ErrorResponse")),
				}),
		},
		"/docs": {
			Get: operation("docs", "Scalar API reference", "docs", nil, nil,
				map[string]OpenAPIResponse{
					"200": mediaResponse("HTML API reference", "text/html", OpenAPISchema{Type: "string"}),
				}),
		},
		"/ucinternal/metrics": {
			Get: operation("cluster", "Proxy the local Uncloud daemon's Prometheus metrics", "internalMetrics", nil, nil,
				map[string]OpenAPIResponse{
					"200": mediaResponse("Prometheus exposition format", "text/plain", OpenAPISchema{Type: "string"}),
					"401": response("Missing or invalid bearer token", ref("ErrorResponse")),
					"503": response("Local machine or its metrics endpoint is unavailable", ref("ErrorResponse")),
				}),
		},
		...
		"/api/v1/services/{id}/logs": {
			Get: operation("services", "Stream service logs", "serviceLogs", append([]OpenAPIParameter{pathParameter("id")}, logParameters(true)...), nil, streamResponses(true)),
		},
	...
	// Probes and the reference page itself are reachable without credentials.
	makePublic(paths["/healthz"].Get)
	makePublic(paths["/docs"].Get)
	return paths
}
...
func mediaResponse(description, mediaType string, schema OpenAPISchema) OpenAPIResponse {
...
func responsesWithStatus(successCode string, success OpenAPIResponse, notFound bool) map[string]OpenAPIResponse {
	...
	addCommonErrors(result, notFound)
	return result
}

// addCommonErrors adds the error responses every authenticated route can return.
// Public routes (/healthz, /docs) clear the 401 by declaring no security.
func addCommonErrors(result map[string]OpenAPIResponse, notFound bool) {
	result["400"] = response("Invalid request", ref("ErrorResponse"))
	result["401"] = response("Missing or invalid bearer token", ref("ErrorResponse"))
	result["500"] = response("Internal server error", ref("ErrorResponse"))
	if notFound {
...
func deployStreamResponses(notFound bool) map[string]OpenAPIResponse {
	result := map[string]OpenAPIResponse{
		"200": streamResponseSchema("Server-Sent deployment events", "DeployComposeEvent"),
		"413": response("Compose file exceeds the size limit", ref("ErrorResponse")),
	}
	addCommonErrors(result, notFound)
	return result
}
...
func streamResponseSchema(description, schemaName string) OpenAPIResponse {
	return mediaResponse(description, "text/event-stream", ref(schemaName))
}
